...
cat host_discovery_results.txt | awk '{print $2}' | grep -v Nmap | while read ip; do
  ...
  nmap --min-rtt-timeout 36ms --initial-rtt-timeout 63ms --max-rtt-timeout 99ms --max-retries 3 --host-timeout 3m --max-scan-delay 6ms -n -Pn -sS -p 0-65535 -A -oA "${ip}_tcp_report" $ip
  ...
  # More aggressive than T5
  nmap --min-rtt-timeout 36ms --initial-rtt-timeout 63ms --max-rtt-timeout 99ms --max-retries 3 --host-timeout 3m --max-scan-delay 6ms -n -sU -A -oA "${ip}_udp_report" $ip
